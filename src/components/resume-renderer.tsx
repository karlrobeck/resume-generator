import { getResumeStyles, type ResumeStyle } from "@/lib/resume-styles";
				...
				<div style={customStyles.contactInfo}>
					...
					{resume.header.contact.linkedin && (
						<>
							<span>•</span>
							<a
								href={resume.header.contact.linkedin}
					...
					{resume.header.contact.github && (
						<>
							<span>•</span>
							<a
								href={resume.header.contact.github}
								style={{ color: "#0066cc", textDecoration: "none" }}
							>
								GitHub
							</a>
					...
					{resume.header.contact.website && (
						<>
							<span>•</span>
							<a
								href={resume.header.contact.website}
								style={{ color: "#0066cc", textDecoration: "none" }}
							>
								{resume.header.contact.website}
							</a>
								...
								{exp.highlights && exp.highlights.length > 0 && (
									<ul style={customStyles.bullets}>
										{exp.highlights.map((highlight) => (
											<li key={highlight.text} style={customStyles.bulletItem}>
												{highlight.text}
											</li>
										))}
									</ul>
					...
					<div>
						{resume.education.map((edu) => (
							<div
								...
								style={customStyles.educationEntry}
							>
								...
								{edu.highlights && edu.highlights.length > 0 && (
									<ul style={customStyles.bullets}>
										{edu.highlights.map((highlight) => (
											<li key={highlight} style={customStyles.bulletItem}>
												{highlight}
											</li>
										))}
									</ul>
				...
				<div style={customStyles.section}>
					...
					<div>
						{resume.skills.map((skillGroup) => (
							<div key={skillGroup.category} style={customStyles.skillGroup}>
								<span style={customStyles.skillCategory}>
									{skillGroup.category}:
								</span>
				...
				<div style={customStyles.section}>
					...
					<div>
						{resume.projects.map((project) => (
							<div key={project.name} style={customStyles.projectEntry}>
								...
								<p style={customStyles.projectDescription}>
									{project.description}
								</p>
			...
			{resume.custom && resume.custom.length > 0 && (
				<div>
					{resume.custom.map((section) => (
						<div key={section.title} style={customStyles.section}>
