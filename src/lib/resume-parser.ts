import type { ResumeAST, ResumeExperience, ResumeEducation, ResumeSkill } from '@/types/resume';
...
export function parseResumeMarkdown(markdown: string): ResumeAST {
  ...
  while (i < lines.length) {
    ...
    if (line?.startsWith('## ')) {
      ...
      } else if (sectionTitle === 'experience' || sectionTitle === 'work experience') {
...
function parseExperienceSection(lines: string[], startIndex: number): ResumeExperience[] {
  ...
  while (i < lines.length && !lines[i]?.trim().startsWith('## ')) {
    ...
    if (line?.startsWith('### ')) {
      ...
      for (let j = 1; j < parts.length; j++) {
        if (parts[j]?.match(/\d{3,4}|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|-|Present/i)) {
          dates = parts[j] || '';
        } else {
          location = parts[j] || '';
        }
...
function parseEducationSection(lines: string[], startIndex: number): ResumeEducation[] {
...
function parseSkillsSection(lines: string[], startIndex: number): ResumeSkill[] {
...
function parseCertificationsSection(lines: string[], startIndex: number): any[] {
  const certs: any[] = [];
  let i = startIndex;
...
function parseProjectsSection(lines: string[], startIndex: number): any[] {
  const projects: any[] = [];
  let i = startIndex;
